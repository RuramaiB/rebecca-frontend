        ...
        <div v-else class="bg-gray-900 border border-gray-800 rounded-xl p-6">
          ...
          <div class="mb-8 p-4 bg-gray-800/50 rounded-lg">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <p class="text-xs text-gray-500">Google Account ID</p>
                <p class="text-sm text-gray-300 truncate">{{ userProfile.googleAccountId || 'N/A' }}</p>
              </div>
              <div>
                <p class="text-xs text-gray-500">Last updated</p>
                <p class="text-sm text-gray-300">{{ formatDate(userProfile.lastUpdated) }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="text-sm" :class="userProfile.emailVerified ? 'text-green-400' : 'text-yellow-400'">
...
const fetchProfile = async () => {
  ...
  try {
    ...
    const email = localStorage.getItem('artistEmail') || localStorage.getItem('adminEmail') || 'botsoruramai@gmail.com'
    
    const response = await fetch(`http://localhost:8080/oauth2/get-profile-by-/${email}`)
